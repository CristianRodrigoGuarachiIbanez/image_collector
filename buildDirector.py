...
from numpy import ndarray, float as Float, asarray, concatenate;
import logging
class BuildDirector:
    """
    ...
    def __recoverListOfDataFrame(self, direction: str) -> None:
        ...
        '''
        path: str = '/trials/'
        for file in listdir(getcwd()+path):
            print(file)
            if (direction):
...
if __name__ == "__main__":
    ...
    # ---------------------------- concatenate
    binocularImgArray: ndarray = concatenate((biL, biR))
    sceneImgArray: ndarray = concatenate((scL, scR))
    print(binocularImgArray.shape)
    print(sceneImgArray.shape)
    # --------------------------- save data
    ...
    imgData: List[ndarray] = [binocularImgArray, sceneImgArray]
    datasetnames: List[str] = ['binocular_data', 'scene_data'];
    groupname: str = 'feature_data'
    ...
    #gen = writer.loadImgDataFromGroup(groupName='feature_data')
    #for g in gen:
    #    print(g.shape)